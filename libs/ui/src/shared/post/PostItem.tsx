...
import { Image } from 'libs/ui/src/components';
 ...
 export const PostItem: React.FC<IProps> = (props) => {
  ...
  return (
    <div className="flex flex-col bg-white tablet:rounded-lg my-4 p-4 first:mt-0 shadow-lg dark:bg-indigo-1000">
      ...
      {/* --- IMAGE GRID ----- */}
      {post.photos?.length !== 0 && <ImageGrid images={post.photos!.map(img => img)} />}
      ...
      {isAuth ? (
        <div className="flex items-center justify-around py-2 border-t border-gray-200 dark:border-indigo-950">
          ...
          <span
            ...
            onClick={handleToggleComment}
          >
            <CommandLineIcon className='h-4 w-4' />&nbsp;Comment
          </span>
        </div>
      ) : (
        <div className="text-center py-2">
          <span className="text-gray-400 text-sm">
            <Link className="font-medium underline dark:text-indigo-400" href='/login'>Login</Link> to like or comment on post.
          </span>
        </div>
      )}
      {/* {isAuth && (
